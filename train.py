...
with tf.compat.v1.Session(config=config) as sess:
    ...
    valid_writer = tf.compat.v1.summary.FileWriter(log_dir_v, sess.graph)
        ...
        for iEpoch in range(epoch_start, opt.nEpoch + 1):
            ...
            for step in tqdm(range(nStep_train)):
                (
                    ...
                    _tar_class_bools,
                    _tar_img,
                ) = DB_train.getBatch(step * nB, (step + 1) * nB)
                feed_dict = {
                    Colla.is_Training: True,
                    Colla.tar_class_idx: _tar_class_idx,
                    Colla.a_img: _a,
                    Colla.b_img: _b,
                    Colla.c_img: _c,
                    Colla.d_img: _d,
                    Colla.targets: _tar_img,
                    Colla.a_mask: _am,
                    Colla.b_mask: _bm,
                    Colla.c_mask: _cm,
                    Colla.d_mask: _dm,
                    Colla.bool0: _tar_class_bools[0],
                    Colla.bool1: _tar_class_bools[1],
                    Colla.bool2: _tar_class_bools[2],
                    Colla.bool3: _tar_class_bools[3],
                }
                # train
            ...
            for step in tqdm(range(nStep_valid)):
                (
                    ...
                    _tar_class_bools,
                    _tar_img,
                ) = DB_valid.getBatch(step * nB, (step + 1) * nB)
                feed_dict = {
                    Colla.is_Training: False,
                    Colla.tar_class_idx: _tar_class_idx,
                    Colla.a_img: _a,
                    Colla.b_img: _b,
                    Colla.c_img: _c,
            ...
            out_train = " %d epoch -- train loss (G / D) : %.4f /  %.4f" % (
                iEpoch,
                loss_G / nStep_train,
                loss_D / nStep_train,
            )
            out_valid = " %d epoch -- valid loss (G / D) : %.4f /  %.4f" % (
                iEpoch,
                vloss_G / nStep_valid,
                vloss_D / nStep_valid,
            )
            ...
            if iEpoch % 1 == 0:
                path_saved = saver.save(
                    sess, os.path.join(ckpt_dir, "model.ckpt"), global_step=iEpoch
                )
